...
const login = async(req, res) => {
    ...
    try {
        ...
        if(!user) return res.status(404).send({ error: "Please create a user account first to login" })
        
        const passwordMatches = bcrypt.compare(password, user.password);
        if(!passwordMatches) return res.status(400).send({ error: "Invalid password" })
